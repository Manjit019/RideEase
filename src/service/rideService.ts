  ...
  try {
    ...
    if (res.status === 201) {
      router.navigate({
        ...
        params: { id: res.data?.rider?._id },
      });
      // return res.data;
    } else {
      console.log("Error creating ride", res.data);
    }
  } catch (error) {
...
export const getMyRides = async (isCustomer : boolean = true) => {
    try {
        ...
        if(filterRides?.length > 0){
            router.navigate({pathname : isCustomer ? "/customer/liveride" : "/rider/liveride", params : {id : filterRides[0]?._id}});
        }
    } catch (error) {
...
export const updateRideStatus = async (rideId : string,status : string) => {
  try {
    const res = await appAxios.patch(`/ride/update/${rideId}`,{status : status})
    return true;
  } catch (error) {
    Alert.alert("Oops!","There was an error, Please try again later.");
